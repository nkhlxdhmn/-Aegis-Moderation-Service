...
PHONE_PATTERN = re.compile(
    r"(?:(?:\+91|0091|0)[\s\-]?)?[6-9]\d{9}\b" r"|(?:\+91[\s\-]?\d{5}[\s\-]?\d{5})"
)
...
def _clip_scores(openclip_similarities: Any) -> tuple[float, float, float]:
    advertising_terms = {_normalize_label(lbl) for lbl in ADVERTISING_PROMPTS}
    affiliate_terms = {_normalize_label(lbl) for lbl in AFFILIATE_PROMPTS}
    social_terms = {_normalize_label(lbl) for lbl in SOCIAL_MEDIA_PROMPTS}
...
def _yolo_promotion_score(yolo_detections: Sequence[Mapping[str, Any]]) -> float:
    object_terms = {_normalize_label(lbl) for lbl in PROMOTION_OBJECT_LABELS}
    score = 0.0
    for det in yolo_detections:
        if not isinstance(det, Mapping):
            continue
        label = _normalize_label(det.get("class") or det.get("label") or det.get("name") or "")
...
def analyze_promotion(
    ...
    promotion_score = _clamp_score(max(advertising_score, affiliate_score, social_media_score))
