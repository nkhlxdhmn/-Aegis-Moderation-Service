...
def check_surya() -> None:
    ...
    try:
        from backend.pipeline.surya_ocr import load_surya, is_available, _predictor
        load_surya()
        if is_available():
...
def check_easyocr() -> None:
    ...
    try:
        from backend.pipeline.easyocr_engine import load_easyocr, _readers
        load_easyocr()
        if _readers:
    ...
    try:
        ...
        if text_classifier.is_available():
            ...
            _record("MuRIL classifier", _WARN,
                    f"disabled â€” weights absent at {model_dir}")
        else:
...
def check_faiss() -> None:
    ...
    try:
        ...
        _record("faiss-cpu import", _WARN,
                "not installed â€” embedding similarity cache disabled")
    except Exception as exc:
...
def check_warmup() -> None:
    ...
    try:
        from backend.model_warmup import warmup_models, model_status
        t0 = time.time()
        warmup_models()
