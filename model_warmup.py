...
GPU allocation:
  cuda:0 — all models (single-GPU default; set VLM_DEVICE env var to
...
def load_surya() -> None:
    ...
    from pipeline.surya_ocr import load_surya as _load
    ok = _load()
    if ok:
...
def load_ocr() -> None:
    ...
    load_surya()
    from pipeline.easyocr_engine import load_easyocr
    load_easyocr()
...
def load_ml_toxicity() -> None:
    """No-op — ML toxicity disabled until Dockerfile upgrades to PyTorch ≥ 2.6."""


def load_llama() -> None:
    """No-op — Llama removed. Kept for backward compatibility."""
...
def warmup_models() -> None:
    ...
    with _warmup_lock:
        try:
            ...
            logger.info("Warmup: ML toxicity skipped (re-enable when torch ≥ 2.6)")
            load_ml_toxicity()

            logger.info("Warmup: text classifier (MuRIL hook — no-op if weights absent)")
        ...
        _models_loaded = True
        _last_error = None
    logger.info("Moderation model warmup completed — all models on GPU")
...
def model_status_detail() -> dict[str, str]:
    ...
    try:
        ...
        detail["llama"] = "error"
    try:
        from pipeline import ml_toxicity as _tox
    ...
    try:
        from pipeline import text_classifier as _tc
...
# ── Legacy aliases (kept for backwards compat with any external callers) ───────
