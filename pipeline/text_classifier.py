...
disabled and classify_text() returns a neutral result — the pipeline
...
def load_text_classifier() -> None:
    ...
    with _classifier_lock:
        ...
        if not _weights_present(model_dir):
            logger.info(
                "Text classifier disabled — weights not found at %s "
        ...
        try:
            ...
            logger.exception("Text classifier load failed — disabling for this session")
            _classifier_disabled = True
...
def classify_text(text: str) -> dict[str, Any]:
    ...
    Returns a dict with keys:
      label       — "abusive" or "non_abusive"
      abuse_score — float in [0, 1]; probability that text is abusive
      disabled    — True when classifier is not loaded (abuse_score will be 0.0)
