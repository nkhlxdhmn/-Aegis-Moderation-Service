"""Central orchestration for MyItihas image moderation signals.
        ...
        for i in range(2):
            if torch.cuda.is_available() and i < torch.cuda.device_count():
                ...
                if total > 0 and (used / total) > 0.90:
                    logger.warning("GPU%d high memory (%.1f%%) — flushing cache", i, used / total * 100)
...
def analyze_image(
    ...
    # ── Stage 2: Smart OCR (GPU0) — Surya primary + EasyOCR fallback ─────────
